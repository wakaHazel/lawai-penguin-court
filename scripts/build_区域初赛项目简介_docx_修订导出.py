...
OUTPUT_PATH = OUTPUT_DIR / "区域初赛提交材料_项目简介_企鹅法庭_修订版.docx"
...
def add_heading(doc: Document, text: str, level: int = 1) -> None:
    ...
    run = paragraph.add_run(text)
    set_run_font(run, "黑体" if level == 1 else "微软雅黑", 15 if level == 1 else 13, bold=True)
...
def add_table(doc: Document, headers: list[str], rows: list[list[str]], *, col_widths_cm: list[float] | None = None) -> None:
    ...
    for idx, header in enumerate(headers):
        ...
        shade_cell(header_cells[idx], "D9E2F3")
        if col_widths_cm:
...
def configure_document(doc: Document) -> None:
    ...
    footer_run = footer_paragraph.add_run("企鹅法庭·区域初赛项目研究报告  第 ")
    set_run_font(footer_run, "宋体", 9, color=RGBColor(0x66, 0x66, 0x66))
    _append_field_run(footer_paragraph, "PAGE")
    footer_suffix = footer_paragraph.add_run(" 页")
    set_run_font(footer_suffix, "宋体", 9, color=RGBColor(0x66, 0x66, 0x66))


def build_cover(doc: Document) -> None:
    for _ in range(4):
        doc.add_paragraph()
    add_title_paragraph(doc, "【D06】法律AI应用创新与实践", 22, spacing_after=10)
    add_title_paragraph(doc, "区域初赛项目研究报告", 20, spacing_after=12)
    add_title_paragraph(doc, "企鹅法庭·沉浸式庭审模拟与法律辅助分析系统", 18, spacing_after=32)
    add_title_paragraph(doc, "基于腾讯系AI工具链的智慧法律系统解决方案研究", 14, bold=False, spacing_after=28)

    meta_lines = [
        "项目类别：智慧法律应用类 / 系统解决方案",
        "文稿属性：科创项目研究报告（用于区域初赛项目简介部分）",
        "文稿内容：项目基础信息、项目需求分析、关键功能说明、项目使用场景、AI工具使用说明（工作流与Prompt）",
        "研究对象：诉前准备、庭审推演与法律辅助分析一体化系统",
        "形成时间：2026年4月",
    ]
    for line in meta_lines:
        add_title_paragraph(doc, line, 12, bold=False, spacing_after=6)
...
def build_abstract(doc: Document) -> None:
    ...
    add_body(
        doc,
        "在技术方案上，项目采用“前端交互层—后端控制层—腾讯元器工作流编排层—法律能力工具层”的组合架构，以腾讯元器承担流程编排与智能体协同，以得理开放平台API和小理AI承担法律检索与专业支撑，以后端状态控制保证多阶段庭审模拟的稳定推进。在应用设计上，系统将民事诉讼典型流程转化为分阶段、可分支、可复盘的文游式互动过程，使用户能够在接近真实法庭语境的环境中进行策略选择、对抗预判和准备校正。研究结果表明，该系统在普通当事人诉前准备、法学生模拟法庭训练以及智慧法律展示等场景中均具有较高的应用适配性和展示价值，能够为法律AI系统从“问答型”向“过程型、策略型、结果沉淀型”演进提供实践样本。",
        indent=False,
    )
    paragraph = doc.add_paragraph()
    paragraph.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
...
def build_section_needs(doc: Document) -> None:
    ...
    add_body(
        doc,
        "赛题对于智慧法律应用的要求，并不是单纯考察一个大模型是否能回答法律问题，而是强调：要依托腾讯系 AI 工具平台，围绕法律场景开发一个具备专业理解、流程设计、技术实现、专业验证和可演示性的智能体、工具或系统。对照这一要求，“企鹅法庭”的需求设计天然契合赛题目标。项目既体现了法律专业场景的真实需求，也充分使用了赛题要求的腾讯元器、腾讯元宝、CodeBuddy、腾讯开悟平台、小理AI和得理开放平台API等工具链。",
    )
    add_body(
        doc,
...
def build_section_scenarios(doc: Document) -> None:
    ...
    add_body(
        doc,
        "对于本次区域初赛及后续答辩而言，项目本身也需要具备良好的“比赛展示场景适配能力”。评委通常会在较短时间内判断一个作品是否真正聚焦问题、是否形成闭环、是否充分使用官方工具链、是否具有创新表达和完成度。因此，项目必须能够在 5 至 8 分钟的演示窗口中，让评委快速看懂产品价值、看到交互过程、理解技术逻辑，并确认系统确实能够输出专业成果。",
    )
    add_body(
        doc,
...
def build_references(doc: Document) -> None:
    ...
    references = [
        "腾讯开悟比赛平台. 【D06】法律AI应用创新与实践赛题说明[EB/OL]. 2026.",
        "腾讯开悟比赛平台. 服创赛D06赛道帮助指引[EB/OL]. 2026.",
        "中华人民共和国全国人民代表大会常务委员会. 中华人民共和国民事诉讼法[S]. 2024修订版.",
        "深圳得理科技有限公司. 得理开放平台API接口说明[EB/OL]. 2026.",
